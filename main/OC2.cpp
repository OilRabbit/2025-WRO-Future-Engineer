...
//             }
//           } else {
//             if (!motor_degree_accel(area2dist_green(blk_area_after_turn) * ENC_PER_CM - 25, (area2dist_green(blk_area_after_turn) * ENC_PER_CM - 25) / 2, (area2dist_green(blk_area_after_turn) * ENC_PER_CM - 25) / 2, tar_power, tar_power + 2)) {
//               // if (is_anticlockwise) 
//               if (clone_blk_xpos - nearestPillarGlobal.xpos < 100) steering_percentage = -(min_xpos_Gcase(nearestPillarGlobal.area) - nearestPillarGlobal.xpos) * 0.8;
//               else steering_percentage = 0;
//             } else {
...
void OC2_slow(double right_ang, int dist_threshold, int power){
  ...
  static int num_block_avoid = 0;
  static int num_block_side = 2;
  static int motor_move_scan = 0;
    ...
    switch (state) {
      ...
      case DETECT_STATE_OC2:
        ...
        if(num_turn == 0){
          ...
          if (motor_move_scan >= 20 && nearestPillarGlobal.colour != NO_COLOUR && nearestPillarGlobal.area >= 5){
          num_block_side = 1;
        } else if (prev_state == TURN_STRAIGHT_STATE_OC2 && nearestPillarGlobal.colour == NO_COLOUR) {
          num_block_side = 1;
        } else {
          num_block_side = 2;
        }
        if (ultra3Dist <= 38 && nearestPillarGlobal.colour == NO_COLOUR && millis() - dash_time > 2500){ // && MOTOR_ENCODER_VALUE > 80
          ...
          motor_move_scan = abs(MOTOR_ENCODER_COUNT) - motor_last_counter;
          if (nearestPillarGlobal.colour == RED){
            if (nearestPillarGlobal.xpos >= min_xpos_Rcase_detect(nearestPillarGlobal.area)){
              if(num_block_avoid != num_block_side){
                pillars_array.push_back(nearestPillarGlobal);
                pillar_front = nearestPillarGlobal;
                prev_state = DETECT_STATE_OC2;
                state = CURVE_BLK_STATE_OC2;
