 ...
 * 
 */
void OC2_pixy2_day1(double right_ang, int dist_threshold, int power){
  ...
  if (!end_game){
    ...
    switch (state) {
          ...
          if (pillar_front.colour == RED){
            if (pillar_front.xpos >= min_xpos_Rcase_detect(pillar_front.area)){
              ...
              if(is_anticlockwise){
                ...
                change_lane_run_dist = change_lane_dist_algo(pillar_front_info.tof1_dist - 20, imu_yaw - tar_ang, change_lane_run_ang);
                Serial.println(pillar_front_info.tof1_dist);
              } else {
                change_lane_run_ang = change_lane_ang(pillar_front_info.tof2_dist - 15);
                change_lane_run_dist = change_lane_dist_algo(pillar_front_info.tof2_dist - 15, imu_yaw - tar_ang, change_lane_run_ang);
            ...
            if (pillar_front.xpos <= min_xpos_Gcase(pillar_front.area)){
              ...
              if(is_anticlockwise){
                ...
                else change_lane_run_ang = change_lane_ang(5 - pillar_front_info.tof2_dist);
                change_lane_run_dist = change_lane_dist_algo(100 - pillar_front_info.tof2_dist , tar_ang - imu_yaw, change_lane_run_ang);
              }
              Serial.println(change_lane_run_ang);
              Serial.println(change_lane_run_dist);
              if (change_lane_run_dist == 0) change_lane_run_dist = 1;//idk
      ...
      case Backward_until_threshold:
        ...
        if (num_turn >= 13){
          if (pillar_front.colour == RED && is_anticlockwise) outer_wall_dist = dist_t2;
          if (pillar_front.colour == GREEN && !is_anticlockwise) outer_wall_dist = dist_t1;
          prev_state = Backward_until_threshold;
          state = (is_anticlockwise) ? anti_in_parking_p1 : ENDING_STATE_OC2;
          reset_encoder();
          tar_ang += (is_anticlockwise == true) ? right_ang : -right_ang;
          break;
        }
      ...
      case CHANGE_LANE_P3_OC2:
        ...
        if (pillar_front.colour == RED) {
          ...
          if (abs(tar_ang - imu_yaw) > 5 && !is_anticlockwise && tar_ang > imu_yaw) steering_percentage = 100;
      ...
      case SKIP_BLK_STATE_OC2:
        ...
        if (pillar_front.colour == RED){
          ...
          else skip_blk_dist = area2dist_red(blk_area_after_turn) * ENC_PER_CM ;
      ...
      case CLK_OUT_PARKING_P1_STATE:
        ...
        if (abs(imu_yaw) < 55){
          steering_percentage = 100;
          motor_move(9);
          if (nearestPillarGlobal.colour == RED && nearestPillarGlobal.area > 20) idk_color = 1;
          else if (nearestPillarGlobal.colour == GREEN && nearestPillarGlobal.area > 20) idk_color = 2;
          break;
        } else {
          reset_encoder();
          prev_state = CLK_OUT_PARKING_P1_STATE;
          Serial.println(idk_color);
          if (idk_color == 1) state = CLK_OUT_PARKING_RED_P1_STATE;
          else if (idk_color == 2) state = CLK_OUT_PARKING_GREEN_P1_STATE;
          else state = CLK_OUT_PARKING_UNK_P1_STATE;
          break;
        }
        break;
      ...
      case CLK_OUT_PARKING_RED_P1_STATE:
        ...
        if (abs(imu_yaw) < 93){
          ...
          state = CLK_OUT_PARKING_RED_P2_STATE;
          break;
        }
        break;

      case CLK_OUT_PARKING_RED_P2_STATE:
        Serial.println("CLK_OUT_PARKING_RED_P2_STATE");
        if (imu_yaw > 15){
          steering_percentage = -100;
          motor_move(9);
          break;
          ...
          reset_encoder();
          prev_state = CLK_OUT_PARKING_RED_P2_STATE;
          state = SCAN_SECTOR_STATE_OC2;
          break;
        }
        break;

      case CLK_OUT_PARKING_GREEN_P1_STATE:
        Serial.println("CLK_OUT_PARKING_GREEN_P1_STATE");
        if (imu_yaw > 10){
          steering_percentage = -100;
          motor_move(9);
          break;
          ...
          motor_stop(BRAKE);
          reset_encoder();
          prev_state = CLK_OUT_PARKING_GREEN_P1_STATE;
          state = FW2CORNER_OC2 ;
          break;
        }
        break;

      case CLK_OUT_PARKING_UNK_P1_STATE:
        if (abs(imu_yaw) < 70){
          steering_percentage = 100;
          motor_move(9);
        } else {
          motor_stop(BRAKE);
          reset_encoder();
          state = CLK_OUT_PARKING_UNK_P2_STATE;
          break;
        }
        break;

      case CLK_OUT_PARKING_UNK_P2_STATE:
        if (abs(imu_yaw) > 10){
          steering_percentage = -100;
          motor_move(9);
        } else {
          motor_stop(BRAKE);
          reset_encoder();
          state = CLK_OUT_PARKING_UNK_P3_STATE;
          break;
        }
        break;

      case CLK_OUT_PARKING_UNK_P3_STATE:
      ...
      // case CLK_OUT_UNKNOWN:
      //   Serial.println("CLK_OUT_UNKNOWN");
      //   if (imu_yaw > 15){
      //     steering_percentage = -100;
      //     motor_move(9);
      ...
      case anti_in_parking_p4:
        idk = (pillar_front.colour == RED ) ? 1 * abs(66.0 - outer_wall_dist) : 0.8 * abs(43.0 - inner_wall_dist);
        Serial.println(idk);
      ...
      case anti_in_parking_p5:
        ha = (pillar_front.colour == RED ) ? 156 : 173;
      ...
      case anti_in_parking_p6:
        if (abs(MOTOR_ENCODER_COUNT) < 14){
          steering_percentage = (pillar_front.colour == RED ) ? -100 : 0;
          motor_move(9);
          break;
        } else {
          motor_stop(BRAKE);
      ...
      case clkw_in_parking_p4:
        park_dist = (pillar_front.colour == RED ) ? 0.7 * abs(45 - inner_wall_dist) : (50 - outer_wall_dist);
      ...
      case clkw_in_parking_p5:
        ha = (pillar_front.colour == RED ) ? 25 : 27;
        if (abs(imu_yaw - tar_ang) > ha){
          steering_percentage = 100;
          motor_move(-9);
          break;
      ...
      case clkw_in_parking_p6:
        if (abs(imu_yaw - tar_ang) > 6){
          steering_percentage = (pillar_front.colour == GREEN ) ? -100 : 50;
          motor_move(9);
          break;
        } else {
          motor_stop(BRAKE);
...
void OC2_pixy2_day0(double right_ang, int dist_threshold, int power){
  // Disable unecessary threads for efficiency
  safeSuspend(blinkledThread);
  safeSuspend(OC1Thread);
    ...
    switch (state) {
        ...
        if (nearestPillarGlobal.colour == RED || nearestPillarGlobal.colour == GREEN){
          ...
          if (pillar_front.colour == RED){
            if (pillar_front.xpos >= min_xpos_Rcase_detect(pillar_front.area)){
              ...
              state = CHANGE_LANE_P1_OC2;
              if(is_anticlockwise){
                // if (num_turn < 4) change_lane_run_ang = change_lane_ang(100 - pillar_front_info.tof1_dist);
                if (num_turn % 4 == 0) change_lane_run_ang = min_val(46, change_lane_ang(105 - pillar_front_info.tof1_dist));
                else change_lane_run_ang = change_lane_ang(110 - pillar_front_info.tof1_dist);
            ...
            if (pillar_front.xpos <= min_xpos_Gcase(pillar_front.area)){
              ...
              state = CHANGE_LANE_P1_OC2;
              if(is_anticlockwise){
                change_lane_run_ang = change_lane_ang(pillar_front_info.tof1_dist - 3);
                change_lane_run_dist = change_lane_dist_algo(pillar_front_info.tof1_dist - 15, tar_ang - imu_yaw, change_lane_run_ang);
              } else {
                if (num_turn % 4 == 0) change_lane_run_ang = min_val(48, change_lane_ang(110 - pillar_front_info.tof2_dist));
                else change_lane_run_ang = change_lane_ang(115 - pillar_front_info.tof2_dist);
      ...
      case UNKNOWN_TURNING_OC2:
        ...
        if (nearestPillarGlobal.colour != NO_COLOUR && nearestPillarGlobal.area >= 4 && ((is_anticlockwise && nearestPillarGlobal.xpos > 75) || (!is_anticlockwise && nearestPillarGlobal.xpos < 225))){
          ...
          steering_percentage = (tar_ang > 0) ? 95 : -100;
          break;
        } else{
          steering_percentage = 0;
          prev_state = UNKNOWN_TURNING_OC2;
      ...
      case CURVE_BLK_STATE_OC2:
        ...
        motor_move(power);
        if (pillar_front.colour == RED){
          if (nearestPillarGlobal.xpos >= min_xpos_Rcase_detect(nearestPillarGlobal.area) + 20) blk_area_after_turn = nearestPillarGlobal.area;
          if (nearestPillarGlobal.xpos >= min_xpos_Rcase_detect(nearestPillarGlobal.area) + 20) {
            steering_percentage = 100;
            break;
          }
          else {
      ...
      case CLK_OUT_PARKING_P2_STATE:
        ...
        if (abs(imu_yaw) > 12){
          ...
          state = CLK_OUT_PARKING_UNK_P1_STATE;
          break;
        }
        break;

      case CLK_OUT_PARKING_UNK_P1_STATE:
        Serial.println("CLK_OUT_PARKING_UNK_P1_STATE");
        if (abs(MOTOR_ENCODER_COUNT) < 32){
          steering_percentage = 0;
          motor_move(-9);
          break;
          ...
          reset_encoder();
          prev_state = CLK_OUT_PARKING_UNK_P1_STATE;
          state = SCAN_SECTOR_STATE_OC2;
          break;
        }
        break;
      ...
      // A parking state if the run direction is clockwise. Make a turn to get into the parking lot
      case clkw_in_parking_p3:
        if (abs(imu_yaw - tar_ang) < 78){
          steering_percentage = -90;
          motor_move(-10);
          break;
        } else {
          motor_stop(BRAKE);
      ...
      // A parking state if the run direction is clockwise. Make a turn to get into the parking for a parallel parking
      case clkw_in_parking_p4:
        park_dist = 60 ; //(idk_color == 1) ? 0.7 * abs(45 - inner_wall_dist) : (50 - outer_wall_dist)
        if (abs(MOTOR_ENCODER_COUNT) < park_dist){
          steering_percentage = 0;
          motor_move(-9);
          // else motor_move(9);
