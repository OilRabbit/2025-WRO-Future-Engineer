      ...
      switch (state) {
        case DETECT_STATE:
          if (num_turn == 0) {
            if (ultra1Dist > dist_threshold){
              ...
              if (nearestPillarGlobal.colour != NO_COLOUR){
                ...
                turn_waittime = 1000;
                state = DASH_BEFORE_TURNING_STATE;
              }
            } else if (nearestPillarGlobal.colour != NO_COLOUR){
              state = CURVE_P1_STATE;
            } else {
              // straight fw
            }
          } else {
            // no need use one of the ultra
          }
          break;
          ...
          case WAIT_TURN_STATE:
            if ((internalClock.read() - milliseconds) < turn_waittime) continue;
            else {
          ...
          case CHECK_FRONT_BLK_STATE:
            if (nearestPillarGlobal.colour == RED && nearestPillarGlobal.colour.x_pos > min_xpos_Rcase(nearestPillarGlobal.colour.area)){
              state = ST_FW_WITH_BLK_STATE;
            } else if (nearestPillarGlobal.colour == GREEN && nearestPillarGlobal.colour.x_pos < min_xpos_Gcase(nearestPillarGlobal.colour.area)) {
              state = ST_FW_WITH_BLK_STATE;
            } else {
